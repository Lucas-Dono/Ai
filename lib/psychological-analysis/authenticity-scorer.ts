 ...
 * Authenticity Scorer
 *
 * Calcula un score de autenticidad (0-100) para perfiles psicológicos basado en:
 * - Consistencia Big Five ↔ Facetas (20%)
 * - Alineación Valores ↔ Traits (15%)
 * - Coherencia Emocional con Neuroticism (15%)
 * - Coherencia Dark Triad ↔ Agreeableness (10%)
 * - Coherencia Apego ↔ Extraversion (10%)
 * - Alineación de Comportamientos Predichos (30%)
 *
 * @version 1.0.0
 */
...
/**
 * Scorer de autenticidad psicológica.
 */
export class AuthenticityScorer {
  /**
   * Calcula el score de autenticidad completo.
   *
   * @param profile - Perfil psicológico enriquecido
   * @returns Score de autenticidad con desglose
   */
  ...
  /**
   * Verifica consistencia entre Big Five y facetas (20%).
   *
   * @returns Score 0-1
   */
  private checkBigFiveFacetsConsistency(profile: EnrichedPersonalityProfile): number {
    ...
    // Score: 0 desviación = 1.0, 50 desviación = 0.0
  ...
  private checkValuesTraitsAlignment(profile: EnrichedPersonalityProfile): number {
    ...
    // Asumimos que perfiles con traits balanceados tienen mejor alineación
    ...
    // Perfiles muy extremos (alta varianza) pueden tener menos alineación
    // Varianza 0 = 1.0, varianza 2500 (max teórico) = 0.5
  ...
  private checkEmotionalCoherence(profile: EnrichedPersonalityProfile): number {
    ...
    if (profile.facets?.neuroticism) {
      ...
      // Debería estar cerca del neuroticism general
      const deviation = Math.abs(profile.neuroticism - avgNeuroticismFacet);
      return Math.max(0, 1 - deviation / 50);
    }

    // Si no hay facetas, verificar si neuroticism está en rango razonable
    // Neuroticism muy extremo (>90 o <10) es menos común
  ...
  private checkDarkTriadCoherence(profile: EnrichedPersonalityProfile): number {
    ...
    // Dark Triad alto debería correlacionar con agreeableness bajo
  ...
  private checkBehaviorAlignment(profile: EnrichedPersonalityProfile): number {
    // Implementación simplificada: verificar coherencia general
