...
/**
 * Monitoreo de fallbacks de Redis
 * Registra cuando el sistema cae a in-memory para alertar de problemas
 */
...
function logRedisFallback(context: string, error?: any) {
  ...
  // Log crítico para alertar a ops/monitoreo
  console.error(`🔴 REDIS FALLBACK [${context}]: Sistema usando rate limiting en memoria. Total fallbacks: ${fallbackMetrics.count}`, error || '');

  // Si hay muchos fallbacks, alertar más fuertemente
  if (fallbackMetrics.count % 100 === 0) {
    console.error(`⚠️⚠️⚠️ ALERTA: ${fallbackMetrics.count} fallbacks de Redis detectados. Revisar conexión urgentemente.`);
  }
}

/**
 * Obtener métricas de fallback (útil para dashboards de monitoreo)
 */
