...
export async function awardPoints(
  ...
  try {
    // Crear o actualizar UserRewards
...
async function checkLevelUp(userId: string): Promise<void> {
  try {
    ...
    // Fórmula: cada nivel requiere level * 100 XP adicionales
...
async function getUserStats(userId: string): Promise<Record<string, number>> {
  try {
    ...
    // Calcular días más largo con un bond
    ...
    // Obtener rewards para streak
    const rewards = await prisma.userRewards.findUnique({
      where: { userId },
    });

    // Obtener notificaciones respondidas
    const respondedNotifications = rewards?.notificationsResponded || 0;

    // Obtener total de hitos (milestone notifications)
...
export async function trackNotificationResponse(
  ...
  try {
    ...
    // Si respondió en menos de 1 hora, dar puntos bonus
    if (responseTime < 1) {
