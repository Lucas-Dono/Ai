/**
 * CONTEXT BUILDER - Construye contexto rico para mensajes proactivos
 *
 * Recopila toda la información relevante para generar un mensaje
 * proactivo natural y personalizado:
 * - Resumen de últimas conversaciones
 * - Topics pendientes
 * - Life Events recientes
 * - Estado emocional histórico
...
export class ContextBuilder {
  /**
   * Construye contexto completo para mensaje proactivo
   */
  async buildContext(
    agentId: string,
    userId: string,
  ...
  private async getLifeEventsContext(
    ...
    // Detectar eventos próximos (próximas 48h)
  ...
  private extractTopics(text: string): string[] {
    // Extracción simple de palabras clave (5+ caracteres, más mencionadas)
    const words = text
      .toLowerCase()
      .split(/\s+/)
      .filter((w) => w.length >= 5);
