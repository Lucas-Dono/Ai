 ...
 *
 * Utilidad para tracking de eventos desde API routes y server components.
 * Se usa para eventos que ocurren en el backend (signup, conversiones, etc.)
 */
 ...
 *
 * Esta función guarda directamente en la base de datos via Prisma.
 * Se usa para eventos que ocurren en API routes (signup, conversiones, etc.)
 *
 * @example
 * ```ts
...
export async function trackServerEvent({
  ...
  try {
    // Enriquecer metadata con timestamp y userId
    ...
    // No queremos que falle el flujo principal por un error de analytics
