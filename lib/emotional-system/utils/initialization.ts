...
export const PERSONALITY_PRESETS = {
  ...
  // Compañero reflexivo y filosófico
  thoughtfulCompanion: {
    personality: {
      openness: 90,
  ...
  // Compañero divertido y espontáneo
  playfulCompanion: {
    personality: {
      openness: 80,
...
export async function createEmotionalAgent(params: {
  ...
  const agent = await prisma.agent.create({
    data: {
      ...
      profile: {}, // Ya no se usa, todo está en emotional system
    },
  });
  ...
  // Inicializar sistema de voz (si está habilitado)
  if (enableVoice) {
    try {
