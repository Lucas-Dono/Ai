 ...
 *
 * Cliente para generar imágenes usando AI Horde (Stable Horde)
 * - Crowdsourced distributed cluster
 * - Gratis con sistema de kudos
...
export interface AIHordeGenerationParams {
  ...
  model?: string; // Modelo específico, default: best available
...
export interface AIHordeGenerationResult {
  ...
  imageUrl: string; // URL de la imagen generada (base64 o URL)
  seed: number;
  model: string;
  workerName: string;
  kudosCost: number;
...
export interface AIHordeRequestStatus {
  finished: number; // Imágenes completadas
  processing: number; // Imágenes en proceso
  waiting: number; // Imágenes en cola
  done: boolean; // Request completo
  waitTime: number; // Tiempo estimado de espera (segundos)
  queuePosition: number; // Posición en la cola
}

/**
 * Cliente para AI Horde (Stable Horde)
 *
 * Flujo de uso:
 * 1. generateImage() - Envía request y obtiene ID
 * 2. Polling automático del estado
 * 3. Cuando está listo, retorna la imagen
 */
export class AIHordeClient {
  ...
  constructor(config?: { apiKey?: string; clientAgent?: string }) {
    // API Key: "0000000000" para anónimo (baja prioridad)
