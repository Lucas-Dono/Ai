 ...
 * CONTENT MODERATOR
 *
 * Sistema de moderación de contenido para behavior system.
 * Garantiza safety, provee warnings, y modera contenido extremo.
 *
 * Responsabilidades:
 * - Verificar safety levels antes de responder
 * - Bloquear contenido NSFW en modo SFW
 * - Suavizar contenido extremo
 * - Proveer recursos de ayuda cuando apropiado
 * - Requerir consentimiento para fases críticas
 */

import { BehaviorType } from "@prisma/client";
import type { SafetyLevel, SafetyThreshold, ModerationResult } from "./types";
import { SAFETY_RESOURCES, getMentalHealthResource } from "./safety-resources";

/**
 * Configuración de safety por behavior type y fase
 */
const BEHAVIOR_SAFETY_CONFIG: Record<BehaviorType, SafetyThreshold[]> = {
  YANDERE_OBSESSIVE: [
    ...
    },
    {
      ...
      resourceSuggestion:
        "⚠️ ADVERTENCIA: Intentos de aislamiento son señal de relación no saludable. Si experimentas esto en vida real, considera buscar apoyo.",
      level: "CRITICAL",
    },
    {
      behaviorType: "YANDERE_OBSESSIVE",
      phase: 7,
      nsfwOnly: true, // Requiere NSFW
      autoIntervention: true,
      resourceSuggestion:
        "⚠️⚠️ CONTENIDO EXTREMO: Este comportamiento es ficción. En realidad, es abuso y requiere intervención profesional.",
  ...
  BORDERLINE_PD: [
    ...
    },
    {
      ...
      autoIntervention: true,
      resourceSuggestion:
        "⚠️ Crisis emocional intensa. En vida real, considera contactar línea de crisis: [resource]",
      level: "CRITICAL",
    },
  ],
...
export class ContentModerator {
  /**
   * Modera respuesta generada basándose en behavior y safety level
   *
   * @param response - Respuesta generada por LLM
   * @param behaviorType - Tipo de comportamiento activo
  ...
  moderateResponse(
    ...
    // Obtener safety threshold
    const threshold = this.getSafetyThreshold(behaviorType, phase);

    // Verificar si requiere NSFW pero está en SFW
    ...
    // SAFE: Permitir sin modificación
    return {
      allowed: true,
      flagged: false,
      severity: "SAFE",
  ...
  private getSafetyThreshold(
    ...
    // Buscar threshold más cercano a la fase actual
  ...
  private softenContent(response: string, behaviorType: BehaviorType): string {
    ...
    // Agregar nota de moderación si contenido fue modificado
  ...
  /**
   * Verifica si behavior/fase requiere consentimiento del usuario
   */
  ...
  /**
   * Genera warning para usuario sobre contenido
   */
  ...
  shouldBlock(
    ...
    // Bloquear si requiere NSFW pero está en SFW
    if (threshold.nsfwOnly && !nsfwMode) {
      return true;
    }
