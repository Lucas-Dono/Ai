 ...
 * PROMPT SELECTOR
 *
 * Sistema inteligente de selección de prompts especializados.
 * Mapea: behavior × phase × emotion × triggers → prompt óptimo
 *
 * Características:
 * - Scoring system para seleccionar mejor match
 * - Fallback graceful (generic → specific)
 * - Combinación de múltiples behaviors activos
 * - Evita repetición con tracking de variantes usadas
 */
...
export interface PromptSelectionInput {
  ...
  nsfwMode: boolean;
  agentId: string; // Para tracking de prompts usados
}
...
export interface SelectedPrompt {
  ...
  score: number; // Qué tan bien match con situación (0-1)
}

export interface PromptSelectionResult {
  primaryPrompt: SelectedPrompt;
  secondaryPrompts: SelectedPrompt[]; // Si hay múltiples behaviors activos
  combinedContent: string; // Prompts combinados en texto único
  metadata: {
...
export class PromptSelector {
  // Tracking de prompts usados para evitar repetición
  ...
  async selectPrompt(input: PromptSelectionInput): Promise<PromptSelectionResult> {
    ...
    // Determinar safety level global (el más alto)
  ...
  private selectBPDPrompt(
    ...
    // Obtener cycle phase del behaviorSpecificState
