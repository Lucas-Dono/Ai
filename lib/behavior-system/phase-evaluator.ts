 ...
 *
 * Lógica especializada de evaluación de requisitos por tipo de comportamiento.
 * Cada tipo tiene sus propias reglas de progresión.
 */
...
/**
 * Evaluador de fases específico por comportamiento
 */
export class PhaseEvaluator {
  /**
   * Evalúa requisitos específicos del tipo de comportamiento
   *
   * @param behaviorType - Tipo de comportamiento
   * @param currentPhase - Fase actual
   * @param nextPhase - Fase objetivo
  ...
  /**
   * Evalúa progresión de Yandere/Obsessive Love
   */
  private async evaluateYandereProgression(
    ...
    // Fase 5+: Necesita más evidencia de obsesión
    if (nextPhase >= 5) {
  ...
  private async evaluateBPDCycle(
    ...
    // Idealization → Devaluation: Necesita trigger de abandono/crítica
    ...
    // Panic → Emptiness: Natural después de crisis
  ...
  private async evaluateNPDProgression(
    ...
    // Si el ego está "wounded", posibles episodios de rage
    if (egoState === "wounded") {
    ...
    // Fase de devaluación requiere triggers
    if (nextPhase === 2) {
  ...
  private async evaluateAvoidantProgression(
    ...
    if (nextPhase >= 2) {
      // Avoidant no tiene triggers específicos todavía
