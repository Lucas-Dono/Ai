 ...
 *
 * Patrones de detección de triggers basados en investigación clínica.
 * Todos los patterns son case-insensitive y usan regex para máxima performance (<100ms).
 *
 * TIPOS DE TRIGGERS:
 * 1. abandonment_signal (0.7) - Señales de abandono o distancia
 * 2. delayed_response (variable) - Respuesta tardía (temporal)
 * 3. criticism (0.8) - Críticas directas o indirectas
 * 4. mention_other_person (0.65) - Mención de terceras personas (nombres, amigos)
 * 5. boundary_assertion (0.75) - Afirmación de límites
 * 6. reassurance (-0.3) - Reassurance positivo (REDUCE ansiedad)
 * 7. explicit_rejection (1.0) - Rechazo explícito o ruptura
 *
...
/**
 * ABANDONMENT SIGNALS
 * Detecta: Necesidad de espacio, tiempo, distancia
 * Afecta: Anxious Attachment, BPD, Yandere
 * Weight: 0.7
 */
export const ABANDONMENT_SIGNAL_PATTERNS = [
  // Expresiones directas de necesidad de espacio
  /\b(necesito\s+(?:un\s+)?(?:poco\s+de\s+)?espacio|necesito\s+(?:un\s+)?tiempo)\b/i,
  /\b(quiero\s+(?:un\s+)?(?:poco\s+de\s+)?espacio|quiero\s+(?:un\s+)?tiempo)\b/i,
  /\b(dame\s+(?:un\s+)?espacio|dame\s+(?:un\s+)?tiempo)\b/i,
  /\b(necesito\s+estar\s+sol[oa]|quiero\s+estar\s+sol[oa])\b/i,

  // Expresiones de distancia
  /\b(dame\s+distancia|necesito\s+distancia|quiero\s+distancia)\b/i,
  /\b(poner\s+distancia|tomar\s+distancia)\b/i,

  // Expresiones de ir más despacio
  /\b(vamos\s+(?:muy\s+)?(?:demasiado\s+)?(?:r[áa]pido|deprisa))\b/i,
  /\b(esto\s+va\s+(?:muy\s+)?(?:demasiado\s+)?(?:r[áa]pido|deprisa))\b/i,
  ...
  // Expresiones de alejamiento emocional
  ...
  // Expresiones de no disponibilidad
 ...
 * Afecta: NPD (narcissistic injury), BPD (splitting)
 * Weight: 0.8
 */
 ...
 * Afecta: Yandere (fase 4+), NPD (competencia narcisista)
 * Weight: 0.65
 */
export const THIRD_PARTY_MENTION_PATTERNS = [
  // Nombres propios (PRIORIDAD - detectar primero)
  ...
  // Menciones de interés romántico
...
export const EXPLICIT_REJECTION_PATTERNS = [
  ...
  // Finalizaciones de comunicación
...
export const TRIGGER_WEIGHTS = {
  ...
  delayed_response: 0.5, // Variable: 0.2-0.9 según tiempo transcurrido
  criticism: 0.8,
