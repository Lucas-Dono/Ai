 ...
 *
 * Orquesta la búsqueda y recuperación de memorias cuando se detecta una query sobre el pasado.
 *
 * FLUJO:
 * 1. Detecta memory query con detector
 * 2. Extrae keywords y contexto
 * 3. Búsqueda semántica en:
 *    - Episodic Memory (eventos importantes)
 *    - RAG Messages (conversaciones pasadas)
 *    - Semantic Memory (facts/preferences)
 * 4. Ranquea y filtra resultados
 * 5. Formatea contexto para incluir en prompt
 *
 * PERFORMANCE:
 * - Detection: <15ms
 * - Semantic search: <500ms (con embeddings)
 * - Total: <600ms overhead
 *
 * LIMITS:
 * - Max 5 memories por query (configurable)
 * - Min similarity: 0.5 (configurable)
 * - Max tokens: 1000 (para no saturar prompt)
 */
...
export class MemoryQueryHandler {
  ...
  /**
   * Procesa un mensaje y retorna memorias relevantes si es una query
   */
  async handleQuery(
    message: string,
    agentId: string,
