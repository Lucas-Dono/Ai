...
export interface MemoryReference {
  ...
  content: string; // Qué se está recordando
  originalTimestamp?: Date; // Cuándo ocurrió originalmente
...
export function detectMemoryReferences(
  ...
  // Estrategia 1: Detección por patrones
  const patternReferences = detectByPatterns(agentMessage);
  references.push(...patternReferences);

  // Estrategia 2: Cross-referencia con memoria recuperada
...
function detectByPatterns(message: string): MemoryReference[] {
  ...
  for (const [type, patterns] of Object.entries(MEMORY_PATTERNS)) {
    for (const pattern of patterns) {
      ...
      if (match) {
        // Extraer el contenido relevante (después del patrón)
        ...
        // Tomar hasta el siguiente punto o 100 caracteres
        ...
        references.push({
          ...
          confidence: 0.7, // Confianza media para pattern matching
        });
  ...
  // Boost confidence si hay múltiples keywords
...
function detectByMemoryContext(
  ...
  // Si el mensaje es similar a alguna memoria recuperada, es probable que esté haciendo referencia
...
function deduplicateReferences(
  ...
  for (const ref of references) {
    ...
    if (!isDuplicate) {
      ...
      // Si es duplicado, boost confidence del existente
...
export async function detectMemoryReferencesWithLLM(
  ...
  // Por ahora, retornar detección básica
