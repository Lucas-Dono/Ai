 ...
 *
 * Middleware central que integra todos los sistemas de seguridad:
 * - Fingerprinting
 * - Threat detection
 * - Honeypots
 * - Tarpit
...
export async function securityMiddleware(
  ...
  try {
    ...
    if (cfg.enableFingerprinting) {
      try {
        fingerprintData = await fingerprintRequest(request, {
          includeGeolocation: false, // Skip para performance
          includeReputation: false,
        });

        // Si está bloqueado, retornar 403 inmediatamente
    ...
    // Esto previene que errores de seguridad rompan la aplicación
    return await handler();
  }
}
...
/**
 * Wrapper específico para honeypot routes
 */
...
export const SecurityPresets = {
  ...
  // Autenticación (detectar brute force)
  authentication: {
    enableFingerprinting: true,
    enableHoneypots: false,
    enableTarpit: true,
