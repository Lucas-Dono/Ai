 ...
 * - Threat scoring y detección de bots
 */
...
export interface RequestPattern {
  ...
  requestPaths: string[]; // Paths accedidos
  requestOrder: string[]; // Orden de recursos solicitados
  avgTimeBetweenRequests: number;
  burstiness: number; // Qué tan "bursty" son los requests (bot detection)
}
...
/**
 * Genera JA3 fingerprint desde TLS handshake
 * JA3 = MD5(SSLVersion,Ciphers,Extensions,EllipticCurves,EllipticCurvePointFormats)
 *
 * Nota: En Node.js/Express no tenemos acceso directo al TLS handshake
 * Esta función es un placeholder para cuando se integre con proxy reverso (nginx/caddy)
 * que puede pasar el JA3 como header personalizado
 */
...
/**
 * Extrae JA3 de headers si el proxy lo proporciona
 * Requiere configuración en nginx/caddy para pasar el JA3
 */
...
export function analyzeUserAgent(userAgent: string): {
  ...
  // User-Agent vacío o muy corto (sospechoso)
  if (!userAgent || userAgent.length < 10) {
    indicators.push('suspicious_ua_length');
  }

  // User-Agent con caracteres extraños
  if (/[<>{}\\|^`]/.test(userAgent)) {
 ...
 * Usa servicios gratuitos como ip-api.com o ipinfo.io
 */
...
/**
 * Verifica reputación de IP contra listas negras conocidas
 */
export async function checkIPReputation(ipAddress: string): Promise<'clean' | 'suspicious' | 'malicious'> {
  ...
  try {
    ...
    // TODO: Implementar integración con AbuseIPDB o similar
...
export function analyzeRequestPattern(pattern: RequestPattern): {
  ...
  if (pattern.avgTimeBetweenRequests < 100) {
    // Requests muy rápidos (< 100ms) probablemente son automated
  ...
  if (pattern.burstiness > 0.8) {
    // Requests muy "bursty" (todos juntos) es típico de bots
    indicators.push('high_burstiness');
    botScore += 25;
  }

  // 3. Request order analysis
  // Los humanos rara vez cargan recursos en orden alfabético perfecto
...
/**
 * Calcula threat score basado en múltiples señales
 */
...
/**
 * Crea o actualiza fingerprint en la base de datos
 */
export async function upsertFingerprint(
  data: FingerprintData,
  threatAnalysis: ThreatAnalysis
) {
  // Generar ID único basado en múltiples señales
 ...
 */
export async function blockFingerprint(fingerprintId: string, reason: string) {
  try {
    return await prisma.clientFingerprint.update({
...
/**
 * Función principal de fingerprinting
 * Analiza el request y retorna fingerprint completo con threat analysis
 */
