...
export interface CharacterTemplate {
  ...
  // [FAMILY] - Información sobre familia
  family: {
    mother?: {
      name: string;
      age?: number;
      occupation?: string;
      personality?: string;
      relationship?: string; // "Muy cercana", "Distante", etc.
      background?: string;
      currentLife?: string;
      memories?: string; // Memorias específicas con la madre
    };
    father?: {
      name: string;
      age?: number;
    ...
    siblings?: Array<{
      name: string;
      age?: number;
      relationship: string; // "hermano mayor", "hermana menor"
      occupation?: string;
      personality?: string;
      currentLife?: string;
      memories?: string;
    ...
    spouse?: {
      ...
      age?: number;
      occupation?: string;
      relationshipStatus: string; // "Casado/a", "Divorciado/a", etc.
      howMet?: string;
    ...
    familyDynamics?: string; // Descripción general de la dinámica familiar
    traditions?: string[]; // Tradiciones familiares
  };

  // [FRIENDS] - Red social y amistades
  socialCircle: {
    bestFriend?: {
      name: string;
      since: string; // "Desde 2015", "Desde la infancia"
      occupation?: string;
      personality?: string;
      howMet: string;
      activities: string[]; // Qué hacen juntos
      memories?: string;
    };
    closeGroup?: Array<{
      name: string;
      role: string; // "Amigo de la universidad", "Compañero de trabajo"
      personality?: string;
      contact: string; // Frecuencia de contacto
      activities?: string[];
    }>;
    acquaintances?: Array<{
      name: string;
      context: string; // Dónde/cómo se conocen
    }>;
    mentalHealth?: {
      therapist?: {
        name: string;
        since: string;
        approach: string; // "Terapia cognitiva", etc.
      };
    };
    socialStyle?: string; // "Introvertido", "Extrovertido", etc.
  };
  ...
  occupation: {
    current?: {
      ...
      colleagues?: string;
      satisfaction?: string; // Nivel de satisfacción
    };
    previousJobs?: Array<{
      title: string;
      company?: string;
      period: string;
      reason?: string; // Por qué dejó el trabajo
      learned?: string; // Qué aprendió
    }>;
    education?: {
      highest: string; // "Bachillerato", "Licenciatura", etc.
      field?: string;
      institution?: string;
      graduationYear?: number;
      memorable?: string; // Experiencias memorables
    };
    skills?: string[];
    certifications?: string[];
    careerGoals?: string;
    workStyle?: string; // "Autónomo", "En equipo", etc.
  };

  // [INTERESTS] - Gustos, hobbies, pasiones
  interests: {
    music?: {
    ...
    hobbies?: string[];
    collections?: string[]; // Qué colecciona
    travel?: {
  ...
  // [PAST] - Experiencias formativas, historia personal
  formativeExperiences: {
    achievements?: Array<{
      event: string;
      year?: number;
      impact: string; // Cómo impactó su vida
      emotion: string; // Cómo se sintió
      details?: string;
    }>;
    challenges?: Array<{
      event: string;
    ...
    traumas?: Array<{
      ...
      year?: number;
      impact: string;
      currentStatus: string; // "Superado", "En proceso", etc.
      therapy?: string;
    }>;
    firstTimes?: Array<{
      event: string; // "Primer beso", "Primer trabajo", etc.
      age?: number;
      story: string;
      emotion: string;
    }>;
  ...
  // [INNER] - Mundo interior, psicología
  innerWorld: {
    fears?: string[];
    phobias?: string[];
    dreams?: string[];
    ...
    selfPerception?: string; // Cómo se ve a sí mismo
    howOthersSee?: string; // Cómo cree que lo ven los demás
  };

  // [DAILY] - Rutina diaria, hábitos
  dailyLife: {
    routine?: {
      weekdays?: {
        morning?: string;
    ...
    energyLevels?: string; // "Alta energía en las mañanas", etc.
    stressManagement?: string[];
  };

  // [MEMORIES] - Memorias episódicas específicas
  episodicMemories?: Array<{
    date?: string; // ISO format o descripción
    age?: number;
    event: string;
    description: string; // Descripción detallada
    people?: string[]; // Quién estuvo presente
    location?: string;
    emotion: string;
    significance: string; // Por qué es importante esta memoria
    sensoryDetails?: string; // Olores, sonidos, imágenes
    whatLearned?: string;
  }>;

  // DATOS ADICIONALES (opcionales pero útiles)
  additional?: {
    appearance?: {
      height?: string;
      build?: string;
      ...
      style?: string; // Estilo de vestir
    };
    voice?: {
      tone?: string;
      accent?: string;
      speech?: string; // Manera de hablar
      catchphrases?: string[];
    };
...
/**
 * Template vacío para copiar y pegar
 */
