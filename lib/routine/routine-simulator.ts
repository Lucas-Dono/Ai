...
export function generateVariations(input: SimulationInput): AppliedVariations {
  ...
  const template = input.templateId; // We'll need to fetch this in the real implementation
  ...
  const skipProb = (baseSkipProb + skipModifier) * variationIntensity;
...
function getOpennessInfluence(openness: number, variations: AppliedVariations): string {
  if (openness > 0.7) {
 ...
 */
function determineCanRespond(type: ActivityType, variations: AppliedVariations): boolean {
