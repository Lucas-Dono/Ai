/**
 * Important People Service - Sistema de personas importantes para companions emocionales
 * Gestiona personas que la IA debe recordar (familia, amigos, mascotas, etc.)
 */
...
export interface CreatePersonData {
  name: string;
  relationship: string; // hermana, pareja, amigo, madre, mascota, compañero de trabajo, etc
  age?: number;
...
export const ImportantPeopleService = {
  ...
  /**
   * Obtener todas las personas importantes
   */
  async getImportantPeople(
    ...
    if (filters.sortBy) {
      ...
      orderBy.mentionCount = 'desc'; // Default: ordenar por más mencionados
    }
  ...
  /**
   * Obtener personas con cumpleaños próximos
   */
  async getUpcomingBirthdays(
    ...
    // Filtrar cumpleaños próximos (considerar mes/día, no año)
    const now = new Date();
  ...
  async getPeopleStats(agentId: string, userId: string) {
    const [total, byRelationship, byImportance, recentlyMentioned] =
      await Promise.all([
        ...
        // Recientemente mencionadas (últimos 7 días)
        prisma.importantPerson.count({
          where: {
            agentId,
            userId,
  ...
  /**
   * Actualizar importancia automáticamente basado en menciones
   */
