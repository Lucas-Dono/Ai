...
/**
 * Sanitizar texto de entrada para prevenir XSS
 */
function sanitizeText(text: string | undefined): string | undefined {
  if (!text) return text;

  // Remover caracteres peligrosos pero mantener el contenido legible
  return text
    .replace(/[<>]/g, '') // Remover < y >
...
export const ReportService = {
  ...
  async reportPost(data: {
    ...
    if (existingReport) {
      throw new Error('Ya has reportado este post');
    }
