...
export class GenreDetector {
  ...
  private hasRomanceIndicators(result: SearchResult, metadata: Record<string, unknown>): boolean {
  ...
  private hasFriendshipIndicators(result: SearchResult, metadata: Record<string, unknown>): boolean {
    const description = result.description?.toLowerCase() || '';
