...
export async function generateProfileEmbeddings(agentId: string): Promise<void> {
  try {
    ...
    // Obtener semantic memory del agente
    ...
    // Generar embedding para cada sección
...
function getSectionData(worldKnowledge: any, command: string): string {
  switch (command) {
    ...
    case KNOWLEDGE_COMMANDS.MEMORIES:
      // Para memorias episódicas, necesitaríamos cargarlas de la DB
      // Por ahora, retornar descripción general
      return 'memorias y recuerdos importantes de eventos pasados';
