 ...
 *
 * Permite a las IAs recordar conversaciones de grupos en chats 1:1 y viceversa
 * mediante búsqueda semántica. Implementa el concepto de "Universo Compartido".
 */
export class CrossContextMemoryService {
  /**
   * Retrieve relevant memories from other contexts
   *
   * Usa búsqueda semántica híbrida que combina:
   * - Similitud semántica (40%)
   * - Importancia del evento (30%)
   * - Recencia (20%)
   * - Tono emocional intenso (10%)
   */
  async retrieveRelevantMemories(params: RetrieveParams): Promise<RelevantMemory[]> {
    try {
      ...
      // - Sean del agente actual
      // - Vengan de otros contextos (grupos o chats 1:1)
      // - No sean del contexto actual
      const memories = await prisma.crossContextMemory.findMany({
        where: {
          ...
          // Excluir el contexto actual para evitar redundancia
        ...
        take: 50, // Pre-filter: Top 50 por importancia/recencia
      });
      ...
      // 3. Calcular similitud semántica para cada memoria
  ...
  private identifySignificantMoments(messages: any[]): any[] {
    ...
    // Por ahora, simplemente tomamos los últimos mensajes si son largos
