 ...
 *
 * Acumula mensajes en una ventana temporal antes de que las IAs respondan.
 * Esto permite:
 * 1. Respuestas más coherentes a múltiples mensajes seguidos
 * 2. Reducción de latencia percibida (typing más realista)
 * 3. Mejor coordinación entre múltiples IAs
 */
...
const BUFFER_CONFIG = {
  BASE_WINDOW_MS: 2000,    // 2 segundos base
  JITTER_MAX_MS: 2000,     // +0-2 segundos de jitter
  MAX_BUFFER_SIZE: 10,     // Máximo mensajes antes de flush forzado
  BUFFER_TTL_SECONDS: 30,  // TTL del buffer en Redis
};
...
class GroupMessageBufferService {
  ...
  private async addMessageRedis(message: BufferedMessage): Promise<void> {
    ...
    // Si alcanzamos el máximo, forzar flush inmediato
