...
// Cache por zona geográfica (geohash), no por ciudad
...
export async function getWeatherForAgent(
  ...
  try {
    // 1. Obtener coordenadas de la ciudad (cachear esto también)
...
async function getCityCoordinates(city: string, country: string): Promise<CityCoordinates> {
  ...
  try {
    ...
    // Fallback: coordenadas aproximadas por país/región
...
async function fetchWeatherForZone(
  ...
  try {
    ...
    // Fallback: clima genérico por latitud
