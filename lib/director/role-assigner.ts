...
class RoleAssigner {
  ...
  async assignRoles(
    ...
    // Cargar relaciones si no están en contexto
    const relations =
      context.activeRelations ||
      (await aiRelationService.getGroupRelations(groupId));

    // Obtener roles requeridos por la escena
  ...
  private calculateRoleScore(
    ...
    // Factor 1: Rotación de protagonistas (evitar monopolios)
    ...
    if (agent.recentParticipation < avgParticipation * 0.5) {
      ...
      score -= 5; // Penalización para IAs dominantes
    }
  ...
  private getRelationScore(
    ...
    for (const [otherRole, otherAgentId] of Object.entries(alreadyAssigned)) {
      // Buscar relación entre agentId y otherAgentId
