...
async function updateUserAnalyticsSummary(userId: string): Promise<boolean> {
  try {
    ...
    if (!existingSummary?.acquisitionDate) {
      // Buscar primera sesión del usuario para attribution
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const BATCH_SIZE = 10; // Procesar 10 usuarios a la vez
    let successCount = 0;
    let failureCount = 0;
    ...
    for (let i = 0; i < activeUserIds.length; i += BATCH_SIZE) {
      ...
      // Procesar batch en paralelo (pero con límite de BATCH_SIZE)
