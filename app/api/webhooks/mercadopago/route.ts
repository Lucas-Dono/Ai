...
function verifyMercadoPagoSignature(req: NextRequest, body: string): boolean {
  ...
  // Construir el manifest según la especificación de MercadoPago
...
async function handlePreApprovalEvent(preapprovalId: string) {
  try {
    ...
    // Sincronizar suscripción con la base de datos
    ...
    // Obtener información del usuario y suscripción para notificaciones
    ...
    switch (preapproval.status) {
      ...
      case "paused":
        // Suscripción pausada por el usuario o por falta de pago
        log.info({ userId, preapprovalId }, 'Subscription paused - keeping current plan');
        // TODO: Crear template de email para suscripción pausada
        break;
  ...
  try {
    ...
    switch (payment.status) {
      ...
      case "rejected":
        ...
        // MercadoPago hace reintentos automáticos según configuración
        // Razones comunes: cc_rejected_insufficient_amount, cc_rejected_bad_filled_card_number, etc.

        // Enviar email notificando el rechazo
        if (user && user.email && payment.transaction_amount) {
      ...
      case "in_process":
        // Pago en proceso (común con medios como boleto bancário)
...
async function handleClaimEvent(claimId: string) {
  try {
    ...
    // Buscar el pago relacionado para obtener el userId
