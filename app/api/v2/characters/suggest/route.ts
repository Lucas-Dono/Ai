  ...
  try {
    ...
    switch (type) {
      case 'traits':
        ...
        responseFormat = 'Array of 5-8 personality trait strings (one word each)';
        break;

      case 'personality':
        prompt = generatePersonalityPrompt(context);
        responseFormat = 'A 2-3 sentence personality description';
        break;

      case 'purpose':
        prompt = generatePurposePrompt(context);
        responseFormat = 'A 2-3 sentence purpose/role description';
        break;

      case 'backstory':
        prompt = generateBackstoryPrompt(context);
        responseFormat = 'A 3-4 sentence backstory';
        break;
