 ...
 *
 * Sincroniza el estado del agente entre Minecraft y la base de datos
 *
 * Usa esto para:
 * - Actualizar posición del agente en el mundo
 * - Sincronizar actividad actual
 * - Reportar eventos (muerte, daño, interacciones)
 * - Obtener estado emocional actualizado
 *
 * Frecuencia recomendada: cada 5-10 segundos, NO cada tick
 */
...
export async function POST(
  ...
  try {
    ...
    // Procesar eventos si hay
    if (events && events.length > 0) {
      await processEvents(agentId, events, agent);
    }

    // Actualizar metadata del agente con estado de Minecraft
 ...
 */
async function processEvents(agentId: string, events: any[], agent: any) {
  for (const event of events) {
    switch (event.type) {
      case 'damage_taken':
        // Actualizar estado emocional (fear, anger)
      ...
      case 'item_given':
        // Regalo recibido, afecta relación
        await recordEpisodicMemory(agentId, {
...
/**
 * Genera sugerencias de comportamiento basadas en estado
 */
