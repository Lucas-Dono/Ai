...
export async function POST(req: NextRequest) {
  try {
    ...
    // TODO: Implementar verificación de rol admin cuando esté disponible
    // Por ahora, verificar por email o ID específico
    const isAdmin = user.email === process.env.ADMIN_EMAIL; // Configurar en .env

    if (!isAdmin) {
      return NextResponse.json(
        { error: 'No tienes permisos para subir versiones del mod' },
        { status: 403 }
      );
    }
    ...
    console.log(`[Mod Upload] Admin ${user.email} uploading version ${version}`);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (!isAdmin) {
      return NextResponse.json(
        { error: 'No tienes permisos para eliminar versiones' },
        { status: 403 }
      );
    }
