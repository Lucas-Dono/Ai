 ...
 * @module services/personality-analysis
 */
...
import { DEFAULT_BIG_FIVE, DEFAULT_BASELINE_EMOTIONS } from '../types';
...
export async function generateMoralSchemas(
  personalityText: string,
  values?: CoreValue[]
): Promise<AnalysisResult<MoralSchema[]>> {
...
Common domains: honesty, loyalty, fairness, harm/care, authority, sanctity, liberty`;
  ...
  try {
    ...
    const schemas = parseJSONResponse<MoralSchema[]>(text);
