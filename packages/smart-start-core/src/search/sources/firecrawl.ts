...
import { SearchSource, SearchResult, SearchOptions, GenreId } from '../../types';
// import { getFirecrawlCreditsTracker } from '@/lib/redis/firecrawl-credits'; // Commented out for platform-agnostic build

// Mock credits tracker for platform-agnostic build
const mockCreditsTracker = {
  hasCreditsAvailable: async () => true,
  getStats: async () => ({ used: 0, limit: 500, remaining: 500, month: new Date().toISOString().slice(0, 7), percentageUsed: 0 }),
  incrementCredits: async (_count: number) => {},
  resetCredits: async () => {},
};
...
export class FirecrawlSource implements SearchSource {
  ...
  private creditsTracker = mockCreditsTracker;
