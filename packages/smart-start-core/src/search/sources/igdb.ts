...
export class IGDBSource implements SearchSource {
  ...
  supportedGenres: GenreId[] = ['roleplay', 'friendship'];
  ...
  async authenticate(): Promise<string> {
    ...
    try {
      ...
      if (!this.accessToken) {
        throw new Error('Failed to get access token from Twitch OAuth');
      }
  ...
  async search(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    try {
      ...
      const characters = (await response.json()) as IGDBCharacter[];
  ...
  private async getImage(imageId: number, token: string): Promise<IGDBImage | null> {
    ...
    try {
      ...
      const images = (await response.json()) as IGDBImage[];
