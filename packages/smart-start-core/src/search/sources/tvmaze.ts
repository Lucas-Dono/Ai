...
export class TVMazeSource implements SearchSource {
  ...
  private async searchCharacters(
    ...
    try {
      ...
      const shows = (await showResponse.json()) as Array<{ score: number; show: TVMazeShow }>;
      ...
      for (const { show } of shows.slice(0, 5)) {
        ...
        try {
          ...
          const cast = (await castResponse.json()) as Array<{
            person: TVMazePerson;
  ...
  private async searchPeople(
    ...
    try {
      ...
      const data = (await response.json()) as Array<{ score: number; person: TVMazePerson }>;
