...
export function LandingDemoChat() {
  ...
  if (showSignupPrompt) {
    return (
      <motion.div
        ...
        className="relative w-full rounded-2xl border border-border shadow-2xl overflow-hidden bg-card"
      >
        <div className="h-[460px] sm:h-[520px] p-6 sm:p-8 flex flex-col items-center justify-center text-center bg-gradient-to-br from-primary/5 to-secondary/5">
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <ul className="text-left text-sm space-y-2 mb-6 max-w-md">
          ...
          <div className="flex flex-col gap-3 w-full max-w-md">
            <Link className="w-full" href={`/registro?fromDemo=true${sessionId ? `&demoSessionId=${sessionId}` : ''}`}>
              <Button size="lg" className="w-full h-12 bg-primary hover:bg-primary/90">
                Crear cuenta gratis
              </Button>
            </Link>
            <Link className="w-full" href={`/login?fromDemo=true${sessionId ? `&demoSessionId=${sessionId}` : ''}`}>
  ...
  return (
    <Card className="relative w-full rounded-2xl border-2 border-border shadow-2xl overflow-hidden bg-card">
      <div className="h-[460px] sm:h-[520px] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-border bg-muted/30">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-gradient-to-br from-primary/20 to-secondary/20">
              <img
        ...
        <div
          ...
          className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 bg-gradient-to-b from-muted/10 to-transparent"
        >
          <AnimatePresence>
            {messages.map((message) => (
              ...
              >
                <div
                  className={`${message.role === "user" ? "max-w-[82%]" : "max-w-[88%]"} ${
                    message.role === "user"
